...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, num_splits, with_ibn, with_se, with_nl, block, layers, non_layers, use_cd=False):
        self.inplanes = 64
        super().__init__()
        ...
        self.use_cd = use_cd
        if use_cd:
            self.cd_conv3 = CDConvBlock(inplanes=256*block.expansion, group=32*block.expansion)
            self.cd_conv2 = CDConvBlock(inplanes=128*block.expansion, group=16*block.expansion)
            self.cd_conv1 = CDConvBlock(inplanes=64*block.expansion, group=8*block.expansion)
