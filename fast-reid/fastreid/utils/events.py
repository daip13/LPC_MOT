...
class TensorboardXWriter(EventWriter):
    ...
    def __init__(self, log_dir: str, window_size: int = 20, **kwargs):
        ...
        self._writer = SummaryWriter(os.path.join(log_dir, time_stamp), **kwargs)
    ...
    def write(self):
        ...
        if storage._vector_data:
            for k, v in storage._vector_data.items():
                self._writer.add_histogram(k, v, storage.iter)

        if storage._img_data:
            for k, v in storage._img_data.items():
                self._writer.add_image(k, v, storage.iter, dataformats='HWC')
...
class EventStorage:
    ...
    @staticmethod
    def tensor_list_for_board(img_list):
        grid_h = len(img_list)
        grid_w = max(len(img_tensors)  for img_tensors in img_list)
        
        height, width, channel = img_list[0][0].shape
        canvas_h = grid_h * height
        canvas_w = grid_w * width
        canvas = np.zeros((canvas_h, canvas_w, channel), dtype=np.uint8)
        for i, img_tensors in enumerate(img_list):
            for j, img_tensor in enumerate(img_tensors):
                offset_h = i * height
                offset_w = j * width
                canvas[offset_h : offset_h + height, offset_w : offset_w + width, :] = img_tensor
        return canvas

    def put_images(self, img_name, img_tensors, labels, weights, l2_dist=None, **kwargs):
        labels_np = labels.cpu().data.numpy()
        weights_np = weights.detach().cpu().numpy()
        if l2_dist is not None:
            l2_dist = l2_dist.detach().cpu().numpy()
        ims = []
        im_np = np.transpose(img_tensors.detach().cpu().numpy(), (0,2,3,1))
        for i in set(labels_np):
            label_mask = (labels_np == i)
            label_indices = np.where(label_mask)[0]
            im_row = []
            for j in label_indices:
                im_np_copy = im_np[j].copy()
                cv2.putText(im_np_copy, str(round(weights_np[j], 3)), (0, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
                if l2_dist is not None:
                    cv2.putText(im_np_copy, str(round(l2_dist[j], 3)), (0, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                im_row.append(im_np_copy)
            ims.append(im_row)
        self._img_data[img_name] = self.tensor_list_for_board(ims)
    ...
    def put_vector(self, name, value):
        '''
        Add histogram to tensorboard
        '''
        self._vector_data[name] = value
